...
/*returns an struct that contains the indeces of a directories 
...
{   dir_children DC;
    ...
    {   if (fs.directory_array[i].parent_id==id)
        {   DC.children_num++; 
            ...
            //printf("children index of %ld:%d\n",id,DC.children_index[DC.children_num-1]);
        }
    }
    //printf("number of directories: %ld\n",fs.sb.directory_number);
    //printf("number of children directories of %ld: %d\n",id,DC.children_num);
    return DC; 
}
...
long int myFS_size_recur(FileSystem fs, int id)
{
    ...
    {   
        ...
        {
            size2=size2+myFS_size_recur(fs,DC.children_index[l]);
        }
        //printf("%ld\n",size2);
        return size2;
    }
...
int myFS_ls_recur(FileSystem fs, unsigned long int id,char * path);
int myFS_ls_recur(FileSystem fs, unsigned long int id,char* path)
{
    dir_children DC=get_dir_children(fs,id);
        ...
        for(int l=0;l<DC.children_num;l++){
            for(int i = 0; i< fs.sb.file_number; i++){
                if(fs.file_array[i].inode.parent_id == DC.children_index[l]){
                    long int dir_index=DC.children_index[l];
                    strcat(path,fs.directory_array[dir_index].name);
                    printf("%s/%s %ldMB\n",path,fs.file_array[i].inode.name,fs.file_array[i].inode.size/1000);
                }
            } 
        myFS_ls_recur(fs,DC.children_index[l],path);
        }
